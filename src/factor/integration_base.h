...
class IntegrationBase
{
  public:
    ...
    {
        dt_buf.push_back(dt);   //这个dt_buf有点恶心  好像是边缘化那边需要用到
        acc_buf.push_back(acc); 
        gyr_buf.push_back(gyr);
        propagate(dt, acc, gyr);
    }

    void repropagate(const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)  // 需要对uwb进行repropagate吗
    {
        sum_dt = 0.0;   
        ...
        for (int i = 0; i < static_cast<int>(dt_buf.size()); i++)
            propagate(dt_buf[i], acc_buf[i], gyr_buf[i]);   //  这里只用到了一次   是对一个frame进行的吗
    }
